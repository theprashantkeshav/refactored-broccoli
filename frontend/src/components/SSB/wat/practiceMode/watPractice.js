...
import { words } from "../allPracticeWords";
  ...
  return (
    <div>
      <Helmet>
        ...
        />
        <title>WAT Practice Set {setNumber}</title>
        <link
          rel="canonical"
      ...
      {end ? (
        ...
        <div className={styles.tat}>
          <div className={styles.tatHeader}>
            <div>
              <p>
                {current + 1}/{words[`${setNumber}`].length}
              </p>
            </div>
            <div>
          ...
          <div className={styles.tatBody}>
            <div className={styles.icon}>
              <i
                onClick={handlePrev}
                className={`fas fa-arrow-circle-left ${styles.WATleftArrow}`}
              ></i>

              {current + 1 === words[`${setNumber}`].length ? (
                <i
                  onClick={handleEnd}
                  class={`fas fa-sign-out-alt ${styles.WATrightArrow}`}
                ></i>
              ) : (
                <i
                  onClick={handleNext}
                  className={`fas fa-arrow-circle-right ${styles.WATrightArrow}`}
                ></i>
              )}
            </div>
            <div className={styles.words}>
              <p>{words[`${setNumber}`][current].text}</p>
            </div>
            <button
            ...
            <div className={styles.watResponse}>
              <p>
                {showResponse ? words[`${setNumber}`][current].response : null}
              </p>
            </div>
          </div>
        </div>
